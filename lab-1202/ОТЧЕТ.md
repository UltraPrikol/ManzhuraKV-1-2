# Отчет по лабораторной работе 1202
...
### Контрольные вопросы
   - В чем заключается архитектура React Native и как работает мост (Bridge)?
        Архитектура заключается в разделении приложения на два основных потока: JavaScript-поток (логика на React) и Нативный поток (отрисовка UI и доступ к API платформы).
        Мост (Bridge) выступает посредником, который позволяет этим потокам обмениваться асинхронными сообщениями в формате JSON.
        JavaScript отправляет через мост команды на выполнение нативных функций, а нативная часть возвращает результат обратно.
   - Какие преимущества дает использование Redux Toolkit для управления состоянием?
        Упрощение кода: Избавляет от написания большого количества шаблонного кода (boilerplate) при создании экшенов и редьюсеров.
        Управление асинхронностью: С помощью createAsyncThunk легко обрабатывать состояния загрузки, успеха и ошибок при работе с внешними данными (например, SQLite).
        Предсказуемость: Обеспечивает единый источник истины для состояния всего приложения, что упрощает отладку.
   - Чем отличается работа с нативными модулями (камера, геолокация) в React Native от нативной разработки?
        В React Native работа идет через единый JavaScript-интерфейс (например, библиотеки Expo), который скрывает различия между iOS и Android.
        В нативной разработке (Swift/Kotlin) требуется писать отдельный код для каждой платформы, используя специфичные системные API.
        React Native требует настройки разрешений (Permissions) через JS-код, который затем транслируется в нативные запросы системы.
   - Как обеспечивается кроссплатформенность кода в React Native?
        Общая кодовая база: Бизнес-логика пишется один раз на JavaScript/TypeScript и остается идентичной для обеих платформ.
        Трансляция компонентов: Абстрактные компоненты (например, <View>, <Text>) при компиляции автоматически заменяются на соответствующие нативные элементы управления Android и iOS.
        Expo SDK: Предоставляет универсальный набор библиотек (камера, геолокация), которые работают одинаково везде, где установлена среда выполнения.