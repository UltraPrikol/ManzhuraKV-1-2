# Отчет по лабораторной работе 0204
...
Освоить принципы композиции и агрегации для построения сложных объектных структур.
 Реализовать механизмы управления связями между объектами, валидации данных и комплексной
 сериализации/десериализации системы.
...
## Практическая часть
 ...
 __team: list[AbstractEmployee] - список сотрудников, работающих над проектом
 (композиция)
 2. Реализуйте в классе Project:
 __init__() с валидацией входных данных
 add_team_member(employee: AbstractEmployee) -> None - добавление сотрудника в
 проект
 ...
 Методы управления отделами: add_department(), remove_department(),
 get_departments()
 Методы управления проектами: add_project(), remove_project(), get_projects()
 get_all_employees() -> list[AbstractEmployee] - получение всех сотрудников
 компании
 find_employee_by_id(employee_id: int) -> Optional[AbstractEmployee] - поиск
 сотрудника по ID
 calculate_total_monthly_cost() -> float - расчет общих месячных затрат на зарплаты
 1 / 4
02_lab0204-Композиция, Агрегация и Работа со сложными структурами.md
 get_projects_by_status(status: str) -> list[Project] - фильтрация проектов по
 статусу
...
02_lab0204-Композиция, Агрегация и Работа со сложными структурами.md
 ...
 find_overloaded_employees() -> list[AbstractEmployee] - поиск перегруженных
 сотрудников
...
developer = Developer(2, "Bob Smith", "DEV", 5000, ["Python", "SQL"], 
"senior") 
salesperson = Salesperson(3, "Charlie Brown", "SAL", 4000, 0.15, 50000) 
# Создание проектов 
ai_project = Project(101, "AI Platform", "Разработка AI системы", "2024-12
31", "active") 
web_project = Project(102, "Web Portal", "Создание веб-портала", "2024-09
30", "planning") 
...
### Выполненные задачи
- [✓] Задача 1: Четкое разделение композиции и агрегации
- [✓] Задача 2: Полноценная валидация всех операций
- [✓] Задача 3: Корректная обработка исключительных ситуаций
- [✓] Задача 4: Полный цикл сериализации/десериализации
- [✓] Задача 5: Генерация содержательных отчетов
...
```bash
Ошибка (ожидаемо): Сотрудник с ID 2 уже есть в компании.
Ошибка изменения статуса (ожидаемо): Неверный статус проекта: invalid_status
Ошибка удаления отдела (ожидаемо): Нельзя удалить отдел, в котором есть сотрудники.
Компания сохранена в company_data.json
Компания загружена из company_data.json
Экспорт отчётов выполнен: employees_report.csv, projects_report.csv, financial_report.txt
Общие ежемесячные затраты: 25000.0
Статистика по отделам: {'DEV': {'name': 'Development', 'employee_count': 2, 'total_salary': 16500.0, 'avg_salary': 8250.0}, 'SAL': {'name': 'Sales', 'employee_count': 1, 'total_salary': 8500.0, 'avg_salary': 8500.0}}
Анализ бюджетов проектов: {101: {'name': 'AI Platform', 'budget': 200000.0, 'team_cost': 16500.0, 'budget_balance': 183500.0}, 102: {'name': 'Web Portal', 'budget': 50000.0, 'team_cost': 7500.0, 'budget_balance': 42500.0}}
Перегруженные сотрудники: [<Developer id=2 name=Bob Smith>]
Назначение sales на AI project: True
Ошибка удаления проекта (ожидаемо): Нельзя удалить проект, над которым работает команда.
Перенос сотрудника 3 из SAL в DEV выполнен
```markdown
